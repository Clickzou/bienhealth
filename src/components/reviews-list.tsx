...
export default function ReviewsList({ reviews }: { reviews: Review[] }) {
  ...
  return (
    <div>
      ...
      {featured.map((r) => (
        ...
        >
          <div className="p-6 sm:p-7 flex flex-col">
            ...
            <p className="mt-3 text-sm sm:text-[15px] text-black/80 leading-relaxed whitespace-pre-line">{r.text}</p>
            <span className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold text-bien-leaf">
              <Star className="h-3.5 w-3.5 fill-bien-leaf" /> Avis avec photo
            </span>
          </div>
      ...
      <div className="columns-1 sm:columns-2 lg:columns-3 gap-5 sm:gap-6">
        {shownRest.map((r) => (
          ...
          >
            ...
            <p className="mt-3 text-sm text-black/80 leading-relaxed whitespace-pre-line line-clamp-5">{r.text}</p>
          </button>
        ))}
      </div>
      ...
      {active && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true" aria-label={`Avis de ${active.name}`}>
          <button aria-label="Fermer" onClick={() => setActive(null)} className="absolute inset-0 bg-bien-forest/50 backdrop-blur-sm" />
          ...
          <div className={`relative bg-white rounded-[1.5rem] ring-1 ring-border bien-shadow overflow-hidden animate-[bien-fade-up_0.3s_ease] max-h-[92vh] w-full ${active.photo ? "max-w-3xl grid sm:grid-cols-2" : "max-w-lg"}`}>
            <button onClick={() => setActive(null)} aria-label="Fermer" className="absolute top-3 right-3 z-10 grid place-items-center h-9 w-9 rounded-full bg-white/90 text-black ring-1 ring-border hover:bg-bien-cream transition-colors">
              <X className="h-5 w-5" />
            </button>

            {active.photo && (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={active.photo} alt={`Photo de l'avis de ${active.name}`} className="w-full h-56 sm:h-full object-cover" />
            )}
            ...
            <div className="p-6 sm:p-8 overflow-y-auto">
              ...
              <p className="mt-4 text-[15px] text-black/85 leading-relaxed whitespace-pre-line">{active.text}</p>
            </div>
          </div>
        </div>
      )}
